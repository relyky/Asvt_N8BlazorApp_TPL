...
internal class SimsBranchRepo : CodeNameRepoBase<CodeName>, ICodeNameRepo
{
  protected override Task<List<CodeName>> ConcreteLoadTask()
  {
    return Task.Run(async () =>
    {
      ...
      // 模擬查詢資料庫
      var codeList = Enumerable.Range(1, 2000).Select(i => new CodeName
      {
        Code = $"BR{i:D4}",
        Name = $"我是銀行B{i / 20:D3}的分行BR{i:D4}",
        Group = $"B{i / 20:D3}" // 模擬所屬銀行代碼
      }).AsList();
